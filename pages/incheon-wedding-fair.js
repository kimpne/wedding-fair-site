...
export default function 인천웨딩박람회({ sheetData }) {
  ...
  return (
    <>
      <Head>
        <title>인천 웨딩박람회 일정 정보</title>
        <meta name="description" content="인천 지역의 최신 웨딩박람회 일정과 정보를 확인하세요." />
      </Head>

      <HeaderNotice />
      <RegionTabs currentRegion="incheon" />
      ...
      <main style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div>
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {safeSheetData
              .filter(row => row && row[1] && row[1].includes('인천'))
              .map((row, index) => (
