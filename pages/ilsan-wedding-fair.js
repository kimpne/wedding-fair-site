...
export default function 일산웨딩박람회({ sheetData }) {
  ...
  return (
    <>
      <Head>
        <title>일산 웨딩박람회 일정 정보</title>
        <meta name="description" content="일산 지역의 최신 웨딩박람회 일정과 정보를 확인하세요." />
      </Head>

      <HeaderNotice />
      <RegionTabs currentRegion="ilsan" />
      ...
      <main style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div>
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {safeSheetData
              .filter(row => row && row[1] && row[1].includes('일산'))
              .map((row, index) => (
